...
class ReportsModel extends Model
{
    ...
    public function getPaymentMethodAnalysis($businessId = null, array $filters = []): array
    {
        ...
        ')
            ->join('categories', 'categories.name = records.category')
            ->join('transactions', 'transactions.business_id = categories.business_id AND transactions.id = records.transaction_id')
    ...
    private function applyFilters($builder, $businessId = null, array $filters = [])
    {
        // Filtro por negocio
        if ($businessId) {
            $builder->where('categories.business_id', uuid_to_bytes($businessId));
        }
